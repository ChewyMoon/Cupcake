// --------------------------------------------------------------------------------------------------------------------
// <copyright file="VectorExtensions.cs" company="ChewyMoon">
    ...
    public static class VectorExtensions
    {
        ...
        public static double Mode(this Vector source)
        {
            var q = from i in source
                    group i by i
                    into g select new { key = g.Key, count = source.Where(d => d == g.Key).Count() };
        ...
        public static Matrix Stats(this Vector x)
        {
            return (from i in x.Distinct().OrderBy(d => d)
                    let q = (from j in x where j == i select j).Count()
                    select new[] { i, q, q / (double)x.Length }).ToMatrix();
        }
        ...
        public static IEnumerable<int> Top(this Vector source, int n)
        {
            if (source.Length < n)
            {
                throw new InvalidOperationException(
                    string.Format("Cannot get top {0} from a {1} length vector", n, source.Length));
            }

            var hash = new Dictionary<int, double>(n);
            for (var i = 0; i < source.Length; i++)
            {
                ...
                else
                {
                    ...
                    {
                        var idx = (from k in hash.Keys orderby hash[k] ascending select k).First();
                        hash.Remove(idx);
                        hash.Add(i, source[i]);
                    }
                }
